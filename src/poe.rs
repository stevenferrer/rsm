...
impl<T: Config> Pallet<T> {
	...
	pub fn revoke_claim(&mut self, caller: T::AccountId, claim: T::Content) -> DispatchResult {
		...
		let owner = self.get_claim(&claim).ok_or("claim does not exists")?;

		// Check that the `owner` matches the `caller`.
		if *owner != caller {
			return Err("claim not owned");
		}
...
mod test {
	...
	fn basic_proof_of_existence() {
		...
		assert_eq!(poe.create_claim("bob", "Hello, world!"), Err("content already claimed"));
